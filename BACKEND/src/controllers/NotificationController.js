...
class NotificationController {
  ...
  async getNotifications(req, res) {
    try {
      ...
      const notifications =
        await this.notificationService.getNotificationsByReceiver(userEmail, [
  ...
  async getUserReceiverNotifications(req, res) {
    try {
      ...
      const notifications =
        await this.notificationService.getNotificationsBySender(userEmail, [
          'booking_confirmation',
          'rejected',
        ]);
  ...
  async markAsRead(req, res) {
    try {
      ...
      const notification =
        await this.notificationService.markNotificationAsRead(id, userEmail);
  ...
  async acceptNotification(req, res) {
    try {
      ...
      const notification = await this.notificationService.acceptNotification(
        notificationId,
        userEmail,
      );
  ...
  async rejectNotification(req, res) {
    try {
      ...
      const notification = await this.notificationService.rejectNotification(
        notificationId,
        userEmail,
      );
    ...
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error: "Access denied. You're not authorized to confirm labs.",
        });
      }
  ...
  async cancelLab(req, res) {
    try {
      ...
      if (
        ...
        return res.status(403).json({
          error: "Access denied. You're not authorized to cancel labs.",
        });
      }
  ...
  async getAttendeesByBookingId(req, res) {
    try {
      ...
      const notifications =
        await this.notificationService.getNotificationsByBookingId(bookingId);

      if (!notifications || notifications.length === 0) {
        return res.status(404).json({
          error: 'No notifications found for the provided bookingId.',
        });
      }
  ...
  async updateNotificationsToReminder(req, res) {
    try {
      ...
      const result =
        await this.notificationService.updateNotificationsToReminder(today);
